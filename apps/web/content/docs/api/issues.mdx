...
it refreshes snapshots without opening or resolving incidents. Native scheduling remains disabled
unless explicitly configured.

## Refresh issue sources

`rescan()` starts the broader rescan pipeline on self-hosted instances and requires an instance
administrator with job-write access. `rescanStatus()` reads the latest scan's stage statuses, summaries,
and failures. A completed stage can still report gaps such as unreachable servers or an offline desktop.

Use `rescan({ healthOnly: true })` to run the existing container/server health watcher and reconcile
incidents without component updates or domain reconciliation. The REST equivalent accepts
`{"healthOnly":true}` in the body of `POST /api/issues/rescan`. Omitting the input keeps the full rescan
behavior. If a scan is already running, the request returns that existing session; follow its progress
with `rescanStatus()`.

A disconnected desktop cannot establish remote server health. The Monitoring page requests a fresh
health check on desktop reconnection, and an unreachable-server incident resolves only after the
daemon answers successfully.
...
| Run applicable issue checkers, or only container/server health. | `issues.rescan(input?)` | `POST /api/issues/rescan`<br />`job:write` · Self-hosted |
